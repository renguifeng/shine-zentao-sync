...
子命令都在 `zentao` CLI。**安装路径取决于方式**：`npx skills add` 通常装到
`.agents/skills/shine-zentao-sync/`（或按 `--agent` 装到 `.claude/skills/`），手动拷贝可放任意处。
先跑 `npx skills list` 确认实际路径，下文示例用该路径替换 `shine-zentao-sync` 前缀
（脚本靠自身位置定位 `lib.sh`，放哪个 skills 目录都能跑；只有 settings.json 里的 hook 命令需要写对路径）。

**`/zentao` 斜杠命令自动安装**：SessionStart hook（接好 hook 即生效）会幂等地把
`templates/zentao.md` 同步到 `~/.claude/commands/zentao.md`——**装完 skill、重开会话后，直接敲
`/zentao <子命令>` 即可**，无需手建命令、无需改任何环境变量（不碰 PATH / `~/.bashrc`）。
若重开一次后 `/zentao` 仍未出现，再开一次会话即可（取决于客户端扫描命令列表的时机）。
下文示例里的 `zentao xxx` 简写，在 Claude 会话内等价于 `/zentao xxx`。
...
写入全局 `~/.claude/zentao.conf`（chmod 600）。再注册首个任务并接 hook。接 hook（粘进目标项目 `.claude/settings.json`；**路径前缀按 `skills list` 的实际安装位置替换**，如 `.agents/skills/...` 或 `.claude/skills/...`）：
